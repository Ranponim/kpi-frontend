...
const validateSettings = (settings, section = null) => {
  ...
  const rulesToCheck = section
    ? { [section]: validationRules[section] }
    : validationRules;
  ...
  Object.entries(rulesToCheck).forEach(([sectionKey, sectionRules]) => {
    ...
    Object.entries(sectionRules).forEach(([fieldKey, rule]) => {
      ...
      if (rule.type === "string" && typeof value !== "string") {
        errors[fieldPath] =
          rule.message || `${fieldKey}는 문자열이어야 합니다.`;
      ...
      if (rule.type === "number") {
        if (rule.min !== undefined && value < rule.min) {
          errors[fieldPath] =
            rule.message || `${fieldKey}는 ${rule.min} 이상이어야 합니다.`;
          logPreference(
            "error",
            `범위 오류: ${fieldPath}는 ${rule.min} 이상이어야 함`
          );
          return;
        }
        if (rule.max !== undefined && value > rule.max) {
          errors[fieldPath] =
            rule.message || `${fieldKey}는 ${rule.max} 이하여야 합니다.`;
          logPreference(
            "error",
            `범위 오류: ${fieldPath}는 ${rule.max} 이하여야 함`
          );
          return;
        }
      }
      ...
      if (rule.type === "array") {
        if (rule.minLength !== undefined && value.length < rule.minLength) {
          errors[fieldPath] =
            rule.message ||
            `${fieldKey}는 최소 ${rule.minLength}개 항목이 필요합니다.`;
          logPreference(
            "error",
            `배열 길이 오류: ${fieldPath}는 최소 ${rule.minLength}개 필요`
          );
          return;
        }
        if (rule.maxLength !== undefined && value.length > rule.maxLength) {
          errors[fieldPath] =
            rule.message ||
            `${fieldKey}는 최대 ${rule.maxLength}개 항목만 허용됩니다.`;
          logPreference(
            "error",
            `배열 길이 오류: ${fieldPath}는 최대 ${rule.maxLength}개만 허용`
          );
          return;
        }
      }
      ...
      if (rule.type === "string") {
        if (rule.minLength !== undefined && value.length < rule.minLength) {
          errors[fieldPath] =
            rule.message ||
            `${fieldKey}는 최소 ${rule.minLength}자 이상이어야 합니다.`;
          logPreference(
            "error",
            `문자열 길이 오류: ${fieldPath}는 최소 ${rule.minLength}자 필요`
          );
          return;
        }
        if (rule.maxLength !== undefined && value.length > rule.maxLength) {
          errors[fieldPath] =
            rule.message ||
            `${fieldKey}는 최대 ${rule.maxLength}자까지 허용됩니다.`;
          logPreference(
            "error",
            `문자열 길이 오류: ${fieldPath}는 최대 ${rule.maxLength}자만 허용`
          );
          return;
        }
      }
...
export const usePreference = () => {
        ...
        Object.entries(newSettings).forEach(([section, sectionSettings]) => {
          if (typeof sectionSettings === "object") {
            Object.entries(sectionSettings).forEach(([key, value]) => {
              logPreference("debug", `설정 업데이트: ${section}.${key}`, {
                value,
              });
  ...
  const resetSettings = useCallback(
    async (sections) => {
      ...
      if (context?.resetSettings) {
        ...
        return {
          success: false,
          error: "resetSettings 함수를 사용할 수 없습니다",
        };
      }
    },
  ...
  const updateSettingsWithValidation = useCallback(
    (newSettings, section = null) => {
      logPreference("info", "설정 업데이트 및 검증 시작", {
        newSettings,
        section,
      });
  ...
  const exportSettings = useCallback(
    (filename = null, partial = null) => {
      ...
      try {
        ...
        link.download =
          filename ||
          `preference-settings-${new Date().toISOString().split("T")[0]}.json`;
...
export const useDashboardSettings = () => {
  ...
  const dashboardSettings = useMemo(() => {
    ...
    if (
      Array.isArray(rawDashboardSettings?.selectedPegs) &&
      rawDashboardSettings.selectedPegs.length > 0
    ) {
  ...
  const dashboardValidationErrors = useMemo(() => {
    return Object.fromEntries(
      Object.entries(validationErrors).filter(([key]) =>
        key.startsWith("dashboardSettings.")
      )
    );
  }, [validationErrors]);

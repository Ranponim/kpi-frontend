...
const Dashboard = () => {
  ...
  const selectedPegs = useMemo(() => {
    return Array.isArray(dashboardSettings?.selectedPegs)
  ...
  const selectedNEs = useMemo(() => {
    return Array.isArray(statisticsSel.selectedNEs)
      ? statisticsSel.selectedNEs
      : [];
  }, [statisticsSel.selectedNEs]);

  const selectedCellIds = useMemo(() => {
    return Array.isArray(statisticsSel.selectedCellIds)
  ...
  // 설정 업데이트 함수들
  const updateDefaultTimeRange = useCallback(
    (value) => {
  ...
  const updateEnableTimeComparison = useCallback(
    (checked) => {
  ...
  const fetchKPIData = useCallback(async () => {
    ...
    // 안전한 기본값 설정
    const safeSelectedPegs =
      selectedPegs.length > 0 ? selectedPegs : ["randomaccessproblem"];
    const safeSelectedNEs =
      selectedNEs.length > 0 ? selectedNEs : ["NVGNB#101086"];
    const safeSelectedCellIds =
      selectedCellIds.length > 0
        ? selectedCellIds.map((id) => parseInt(id))
        : [8418];
    ...
    try {
      ...
      if (
        ...
        logDashboard("info", "Time1 API 파라미터 상세", {
          ...
          time1DurationMinutes: `${
            Math.abs(new Date(time1End) - new Date(time1Start)) / (1000 * 60)
          }분`,
          safeSelectedPegs,
          safeSelectedNEs,
          safeSelectedCellIds,
        ...
        logDashboard("info", "Time2 API 파라미터 상세", {
          ...
          time2DurationMinutes: `${
            Math.abs(new Date(time2End) - new Date(time2Start)) / (1000 * 60)
          }분`,
          timeComparison: {
            time1Start,
            time1End,
            ...
            timeOverlap:
              new Date(time1End) > new Date(time2Start)
                ? "시간대 겹침"
                : "시간대 분리",
          },
        });
        ...
        const calculateDerivedPegsForDataset = (baseData, timeLabel) => {
          if (
            ...
            derivedPegSettings.formulas.length === 0
          ) {
            return baseData;
          }

          const activeDerivedPegs = safeSelectedPegs.filter((peg) =>
            derivedPegSettings.formulas.some(
              (formula) =>
                formula.active &&
                formula.name.replace(/[^a-zA-Z0-9_]/g, "_").toLowerCase() ===
                  peg
            )
          );
            ...
            baseData[firstBasicPeg].forEach((row, index) => {
              ...
              Object.keys(baseData).forEach((pegKey) => {
                ...
                if (
                  pegData &&
                  pegData[index] &&
                  pegData[index].entity_id === entityId
                ) {
              ...
              Object.entries(calculatedDerivedPegs).forEach(
                ([derivedPegKey, value]) => {
        ...
        logDashboard("info", "Time2 데이터 상세 분석", {
          ...
          time2ResponseDataKeys: time2Response?.data
            ? Object.keys(time2Response.data)
            : [],
          ...
          targetKpiDataLength: Array.isArray(time2KpiData)
            ? time2KpiData.length
            : "N/A",
          targetKpiDataSample:
            Array.isArray(time2KpiData) && time2KpiData.length > 0
              ? time2KpiData.slice(0, 3)
              : [],
          comparison: {
            time1DataLength: Array.isArray(time1Data[targetKpiKey])
              ? time1Data[targetKpiKey].length
              : "N/A",
            time2DataLength: Array.isArray(time2KpiData)
              ? time2KpiData.length
              : "N/A",
            difference:
              (Array.isArray(time1Data[targetKpiKey])
                ? time1Data[targetKpiKey].length
                : 0) - (Array.isArray(time2KpiData) ? time2KpiData.length : 0),
          },
        });
        ...
        logDashboard("debug", "데이터 저장 후 상태", {
          ...
          time2DataSample:
            Object.keys(time2Data).length > 0
              ? {
                  firstKey: Object.keys(time2Data)[0],
                  firstKeyDataLength:
                    time2Data[Object.keys(time2Data)[0]]?.length || 0,
                }
              : null,
        });
        ...
        logDashboard("info", "Time1/Time2 데이터 fetching 완료", {
          ...
          time1TotalRows: Object.values(time1Data).reduce(
            (sum, arr) => sum + (arr?.length || 0),
            0
          ),
          time2KpiCount: Object.keys(time2Data).length,
          time2TotalRows: Object.values(time2Data).reduce(
            (sum, arr) => sum + (arr?.length || 0),
            0
          ),
        ...
        const start = new Date(
            ...
            (dashboardSettings?.defaultHours || 1) * 60 * 60 * 1000
        );
        ...
        logDashboard("info", "일반 모드 데이터 fetching 완료", {
          ...
          totalRows: Object.values(dataByKpi).reduce(
            (sum, arr) => sum + (arr?.length || 0),
            0
          ),
    ...
    selectedNEs,
    selectedCellIds,
    dashboardSettings?.defaultHours,
    defaultNe,
    defaultCellId,
    enableTimeComparison,
    time1Start,
  ...
  // LLM 분석 핸들러 (V2 API 호출 - MCP 직접 실행)
  const handleLLMAnalysis = useCallback(async () => {
    if (!inputCompleted.time1 || !inputCompleted.time2) {
    ...
    try {
      ...
      logDashboard("info", "LLM 분석 V2 시작");

      // DB 설정 (Preference에서)
      const dbConfig = pref?.databaseSettings || {};
      
      // 선택된 NE와 Cell ID (Statistics 설정에서 가져오거나 사용자 입력)
      const selectedNE = selectedNEs.length > 0 ? selectedNEs[0] : null;
      const selectedCell = selectedCellIds.length > 0 ? selectedCellIds[0] : null;
      
      // 시간 형식 변환: "2025-01-19T00:00" → "2025-01-19_00:00"
      const formatTime = (isoTime) => {
        if (!isoTime) return "";
        return isoTime.replace("T", "_").substring(0, 16); // YYYY-MM-DD_HH:MM
      };
      
      // API 요청 데이터
      const requestData = {
        n_minus_1: `${formatTime(time1Start)}~${formatTime(time1End)}`,
        n: `${formatTime(time2Start)}~${formatTime(time2End)}`,
        ne_id: selectedNE,
      ...
      logDashboard("info", "LLM 분석 V2 요청 데이터", {
        ...requestData,
      ...
      toast.error(
        `LLM 분석 실패: ${error?.response?.data?.detail || error?.message || "알 수 없는 오류"}`
      );
    } finally {
  ...
  const buildChartDataByLayout = useCallback(
    (kpiKey) => {
      ...
      const time2Rows =
        enableTimeComparison && time2Data
          ? Array.isArray(time2Data[kpiKey])
            ? time2Data[kpiKey]
            : []
          : [];
      ...
      if (enableTimeComparison) {
        logDashboard("debug", "buildChartDataByLayout - Time2 데이터 분석", {
          ...
          kpiKeyInTime2Data: time2Data
            ? time2Data.hasOwnProperty(kpiKey)
            : false,
      ...
      if (chartLayout === "byPeg") {
        ...
        let timeIndex = 0;
        let isFirstTime1 = true;
        let isFirstTime2 = true;

        // 안전한 시간 포맷팅 함수
        const formatTime = (date, isFirst = false) => {
        ...
        flatRows.forEach((row) => {
          ...
          // 첫 번째 Time1에만 MM/DD HH:MM, 이후에는 HH:MM만 표시
          const displayTime = formatTime(originalTime, isFirstTime1);
          isFirstTime1 = false;
          ...
          const entityKey = enableTimeComparison
            ? `${row.entity_id}_Time1`
            : row.entity_id;
        ...
        if (enableTimeComparison && time2Rows.length > 0) {
          ...
          time2Rows.forEach((row) => {
            ...
            // 첫 번째 Time2에만 MM/DD HH:MM, 이후에는 HH:MM만 표시
            const displayTime = formatTime(originalTime, isFirstTime2);
            isFirstTime2 = false;
          ...
          logDashboard("debug", "Time2 데이터 처리 완료", {
            ...
            time2KeysInGrouped: Object.keys(groupedByTime).filter((key) =>
              key.startsWith("T2_")
            ).length,
          });
        } else {
        ...
        logDashboard("info", "Time1/Time2 데이터 구성 완료", {
          ...
          sampleDataPoints: sortedData.slice(0, 3).map((point) => ({
            ...
            entityCount: Object.keys(point).filter((k) => !k.startsWith("_"))
              .length,
          ...
          time1Entities:
            flatRows.length > 0
              ? Object.keys(flatRows[0]).filter((k) => k !== "timestamp")
              : [],
          time2Entities:
            time2Rows.length > 0
              ? Object.keys(time2Rows[0]).filter((k) => k !== "timestamp")
              : [],
          // Time2 렌더링 문제 디버깅용
          time2EntityKeys:
            time2DataPoints.length > 0
              ? Object.keys(time2DataPoints[0]).filter(
                  (k) => !k.startsWith("_")
                )
              : [],
        ...
        logDashboard("debug", "buildChartDataByLayout 최종 반환 데이터", {
          ...
          returnedData: sortedData.slice(0, 3).map((data) => ({
            ...
            entityCount: Object.keys(data).filter((k) => !k.startsWith("_"))
              .length,
        ...
        Object.keys(byEntity).forEach((entity) => {
          result[entity] = Object.values(byEntity[entity]).sort(
            (a, b) =>
              new Date(a.time.replace(" (T2)", "")) -
              new Date(b.time.replace(" (T2)", ""))
          );
        });
        return result;
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Badge variant="default">선택된 PEG: {selectedPegs.join(", ")}</Badge>
        {enableTimeComparison && (
      ...
      <Dialog
        open={zoomed.open}
        onOpenChange={(open) => setZoomed((prev) => ({ ...prev, open }))}
      >

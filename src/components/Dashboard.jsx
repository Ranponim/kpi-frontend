...
const Dashboard = () => {
  ...
  const selectedPegs = Array.isArray(dashboardSettings?.selectedPegs)
  ...
  const selectedNEs = Array.isArray(statisticsSel.selectedNEs)
    ? statisticsSel.selectedNEs
    : [];
  const selectedCellIds = Array.isArray(statisticsSel.selectedCellIds)
  ...
  const fetchKPIData = React.useCallback(async () => {
    ...
    try {
      ...
      const start = new Date(
        end.getTime() - (dashboardSettings?.defaultHours || 1) * 60 * 60 * 1000
      );

      // 기본 PEG만 API로 요청 (Derived PEG 제외)
      const basicPegs = selectedPegs.filter(
        (peg) =>
          !derivedPegSettings.formulas?.some(
            (formula) =>
              formula.active &&
              formula.name.replace(/[^a-zA-Z0-9_]/g, "_").toLowerCase() === peg
          )
      );
